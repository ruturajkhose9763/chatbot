...
async function speakText(text, onDone, alreadyTranslated) {
  ...
  // Try the natural ElevenLabs voice first. If it's not configured on the
  // server, or the request fails for any reason, we fall straight through
  // to the browser's own built-in voice below — voice mode never breaks.
...
async function speakWithElevenLabs(text, onDone) {
  try {
    const res = await fetch('/api/tts', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ text, voice: currentElevenLabsVoice })
    });
