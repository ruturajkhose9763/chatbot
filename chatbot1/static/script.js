...
async function sendMessage(text, onSpokenEnd) {
  ...
  try {
    ...
    speakText(data.reply, onSpokenEnd);
  } catch (err) {
    typingEl.remove();
...
//
// Voice LANGUAGE selector (Hindi / English / Marathi / Auto): jo bhi
// language chuno, reply usi mein bola jaata hai — agar reply text kisi
// aur language mein tha, pehle server par (Gemini se) translate hota hai,
// phir bola jaata hai. Agar translation fail ho jaye (no internet / no
// API key), to seedha original text hi bol dete hain — kabhi rukta nahi.
// =================================================================
let voiceModeActive = false;

const voiceSelect = document.getElementById('voiceSelect');
let voiceLangPref = localStorage.getItem('voiceLangPref') || 'auto';
if (voiceSelect) {
  voiceSelect.value = voiceLangPref;
  voiceSelect.addEventListener('change', () => {
    voiceLangPref = voiceSelect.value;
    localStorage.setItem('voiceLangPref', voiceLangPref);
  });
}

// ---------- Pick the best-sounding free voice already installed in the browser ----------
let _cachedVoices = {}; // keyed by lang preference, so we don't re-search every time
function pickBestVoice(langPref) {
  ...
  if (langPref === 'hi') {
    return (
      ...
      voices.find((v) => v.name === 'Google हिंदी') ||
      voices.find((v) => v.lang === 'hi-IN') ||
      voices.find((v) => v.lang && v.lang.startsWith('hi'))
    ) || pickBestVoice('en'); // no Hindi voice installed — fall back to English rather than silence
  }
  if (langPref === 'mr') {
    // Marathi TTS voices are rare on most devices; use one if present,
    // otherwise Hindi (closest phonetically), then English.
    return (
  ...
  // 'en' or 'auto' — known high-quality "natural" English voices, best first.
...
if ('speechSynthesis' in window) {
  window.speechSynthesis.onvoiceschanged = () => { _cachedVoices = {}; };
}

// ---------- Fix common mispronunciations before speaking ----------
// Browser TTS reads abbreviations, "&", phone numbers, etc. very oddly.
// This cleans the text up right before it's spoken (display text is
// never touched — only what gets read out loud).
function prepareForSpeech(text) {
  ...
  const replacements = [
    ...
    [/\bMr\.\s?/g, 'Mister '],
    [/\bMs\.\s?/g, 'Miss '],
    [/\bHOD\b/g, 'Head of Department'],
    [/\bTPO\b/g, 'Training and Placement Officer'],
    [/\bS&H\b/g, 'Science and Humanities'],
    [/\bEngg\.?\b/gi, 'Engineering'],
    [/\bSEDCOE\b/g, 'S E D C O E'],
    [/\bCR\b/g, 'Class Representative'],
    [/&/g, ' and '],
    [/₹/g, 'Rupees '],
    [/%/g, ' percent'],
  ];
  ...
  // Read 10-digit phone numbers digit-by-digit for clarity ("nine eight
  // seven...") — but leave fee amounts, institute codes etc. as normal
  // numbers, since those should be read as a quantity, not spelled out.
  t = t.replace(/\b(\d{10})\b/g, (match) => match.split('').join(' '));
...
function speakText(text, onDone) {
  ...
  // "Auto" = speak the reply exactly as written, no translation needed.
  if (voiceLangPref === 'auto') {
    const plain = prepareForSpeech(text);
    const utterance = new SpeechSynthesisUtterance(plain);
    utterance.rate = 0.98;
    utterance.pitch = 1.02;
    if (!_cachedVoices.auto) _cachedVoices.auto = pickBestVoice('auto');
    if (_cachedVoices.auto) utterance.voice = _cachedVoices.auto;
  ...
  // Hindi / English / Marathi picked — translate first so the reply is
  // actually spoken in that language, not just pronounced with an accent.
  setVoiceStatus('thinking');
  fetch('/api/translate-for-speech', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ text, lang: voiceLangPref }),
  })
    .then((r) => r.json())
    .then((data) => {
      const finalText = (data && data.text) ? data.text : text;
      const plain = prepareForSpeech(finalText);
      const utterance = new SpeechSynthesisUtterance(plain);
      utterance.rate = 0.98;
      utterance.pitch = 1.02;
      if (!_cachedVoices[voiceLangPref]) _cachedVoices[voiceLangPref] = pickBestVoice(voiceLangPref);
      if (_cachedVoices[voiceLangPref]) utterance.voice = _cachedVoices[voiceLangPref];
      utterance.onstart = () => setVoiceStatus('speaking');
      utterance.onend = () => { if (onDone) onDone(); };
      utterance.onerror = () => { if (onDone) onDone(); };
      window.speechSynthesis.cancel();
    ...
    .catch(() => {
      // Offline or translation failed — still speak, just in original text,
      // so voice mode never silently breaks.
      const plain = prepareForSpeech(text);
      const utterance = new SpeechSynthesisUtterance(plain);
      utterance.rate = 0.98;
      if (!_cachedVoices[voiceLangPref]) _cachedVoices[voiceLangPref] = pickBestVoice(voiceLangPref);
