...
if (voiceLangSelect) {
  ...
  voiceLangSelect.addEventListener('change', () => {
    ...
    _cachedVoice = pickBestVoice(currentVoiceLang, currentVoiceGender);
    window.speechSynthesis.cancel();
  });
}

// ---------- Voice gender preference: 'female' (default) or 'male' ----------
let currentVoiceGender = localStorage.getItem('voiceGender') || 'female';
const voiceGenderSelect = document.getElementById('voiceGenderSelect');
if (voiceGenderSelect) {
  voiceGenderSelect.value = currentVoiceGender;
  voiceGenderSelect.addEventListener('change', () => {
    currentVoiceGender = voiceGenderSelect.value;
...
// Known voice names, grouped by how they usually sound (browsers don't
// reliably expose gender as data, so we match on well-known voice names).
const FEMALE_VOICE_NAMES = [
  'Google UK English Female', 'Google हिन्दी', 'Google हिंदी', 'Google US English',
  'Microsoft Heera Online (Natural) - English (India)', 'Microsoft Heera - English (India)',
  'Microsoft Zira - English (United States)', 'Samantha', 'Microsoft Swara Online (Natural) - Hindi (India)',
];
const MALE_VOICE_NAMES = [
  'Google UK English Male',
  'Microsoft Ravi - English (India)', 'Microsoft Ravi Online (Natural) - English (India)',
  'Microsoft David - English (United States)', 'Daniel', 'Microsoft Madhur Online (Natural) - Hindi (India)',
];

// ---------- Pick the best-sounding free voice already installed in the browser ----------
let _cachedVoice = null;
function pickBestVoice(lang, gender) {
  if (!('speechSynthesis' in window)) return null;
  const voices = window.speechSynthesis.getVoices();
  if (!voices.length) return null;
  gender = gender || 'female';
  const nameList = gender === 'male' ? MALE_VOICE_NAMES : FEMALE_VOICE_NAMES;

  // 1) Try an exact gender+language match from the known-name lists first.
  for (const name of nameList) {
    const v = voices.find((v) => v.name === name);
    if (v && (lang === 'en' || (v.lang && v.lang.toLowerCase().startsWith(lang)))) return v;
  }

  if (lang === 'hi') {
    return (
      voices.find((v) => v.lang === 'hi-IN' && nameList.some((n) => v.name.includes(n.split(' ')[0]))) ||
      voices.find((v) => v.lang === 'hi-IN') ||
      voices.find((v) => v.lang && v.lang.startsWith('hi')) ||
      pickBestVoice('en', gender)
    );
  }
  if (lang === 'mr') {
    return (
  ...
  // 2) Fall back to the previous best-effort logic, still respecting language.
  for (const name of nameList) {
    const v = voices.find((v) => v.name === name);
    if (v) return v;
  }
...
if ('speechSynthesis' in window) {
  window.speechSynthesis.onvoiceschanged = () => { _cachedVoice = pickBestVoice(currentVoiceLang, currentVoiceGender); };
  _cachedVoice = pickBestVoice(currentVoiceLang, currentVoiceGender);
}
...
async function speakText(text, onDone) {
  ...
  const voice = _cachedVoice || pickBestVoice(currentVoiceLang, currentVoiceGender);
