...
// Full proper-noun names that browser TTS engines commonly mispronounce.
// Each is replaced with a syllable-broken, phonetically-spelled version
// (only for speech — the on-screen text is never changed).
const NAME_PRONUNCIATIONS = {
  'Samajbhushan Eknathrao Dhakane College of Engineering, Shevgaon':
    'Sa-maj-bhoo-shan Ek-nath-rao Dha-ka-ne College of Engineering, Shev-gaon',
  'Samajbhushan Eknathrao Dhakane':
    'Sa-maj-bhoo-shan Ek-nath-rao Dha-ka-ne',
  'Shevgaon': 'Shev-gaon',
  'Dhakane': 'Dha-ka-ne',
  'Eknathrao': 'Ek-nath-rao',
};

function fixPronunciation(text) {
  let out = text;
  // Fix proper-noun names first (longest phrases first, so the full college
  // name gets replaced before its individual words do).
  Object.keys(NAME_PRONUNCIATIONS)
    .sort((a, b) => b.length - a.length)
    .forEach((name) => {
      const re = new RegExp(name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g');
      out = out.replace(re, NAME_PRONUNCIATIONS[name]);
    });
