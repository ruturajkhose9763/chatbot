...
// =================================================================
// SPLASH SCREEN (0-100% counter, then auto-remove so it never blocks clicks)
// =================================================================
(function () {
  ...
  const DURATION = 2600; // matches the CSS fade-out delay
...
function makeVoiceFriendly(text) {
  let out = text;
  // Strip emojis entirely — TTS engines either skip them awkwardly or
  // occasionally try to "read" them, both sound odd.
  out = out.replace(/[\u{1F300}-\u{1FAFF}\u{2600}-\u{27BF}\u{2190}-\u{21FF}]/gu, '');
  // Each bullet line becomes its own short spoken sentence instead of
  // being smooshed into one long comma-separated run-on line.
  out = out
    .replace(/\n\s*[•●▪-]\s*/g, '. ')
    .replace(/—/g, ', ')
    .replace(/\n{2,}/g, '. ')
...
async function speakText(text, onDone, alreadyTranslated) {
  ...
  let plain = makeVoiceFriendly(text.replace(/[*_#`]/g, ''));
