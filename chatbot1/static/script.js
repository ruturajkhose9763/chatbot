...
const AI_ORB_HTML = '<div class="ai-orb"><svg viewBox="0 0 24 24"><path d="M12 2c0 4 1 8 4 10-3 2-4 6-4 10 0-4-1-8-4-10 3-2 4-6 4-10z" fill="#fff"/></svg></div>';
...
function addMessage(text, sender) {
  ...
  const avatarContent = sender === 'user' ? '🧑' : AI_ORB_HTML;
  msg.innerHTML = `
...
function addTyping() {
  ...
  msg.innerHTML = `
    <div class="avatar">${AI_ORB_HTML}</div>
...
// True "talk to the assistant" conversation flow:
//   tap mic -> speak -> auto-sends -> bot replies -> bot speaks reply out loud
// =================================================================
let voiceOutputEnabled = false;
let voiceConversationMode = false; // becomes true once the user uses the mic

function speakText(text) {
  if (!('speechSynthesis' in window)) return;
  if (!voiceOutputEnabled && !voiceConversationMode) return;
...
(function () {
  ...
  if (voiceToggle) {
    voiceToggle.addEventListener('click', () => {
      ...
      if (!voiceOutputEnabled) { window.speechSynthesis.cancel(); setOrbState('idle'); }
    });
  }
  ...
  recognition.addEventListener('result', (e) => {
    ...
    // Speaking via mic means the user wants a spoken conversation — auto-enable voice replies
    voiceConversationMode = true;
    if (voiceToggle && !voiceOutputEnabled) {
      voiceOutputEnabled = true;
      voiceToggle.textContent = '🔊';
      voiceToggle.title = 'Voice replies ON — click to mute';
    }
    // Auto-send the spoken question, just like a real conversation
  ...
  recognition.addEventListener('end', () => { micBtn.classList.remove('mic-listening'); setOrbState('idle'); });
  recognition.addEventListener('error', () => { micBtn.classList.remove('mic-listening'); setOrbState('idle'); });

  micBtn.addEventListener('click', () => {
    window.speechSynthesis.cancel(); // stop bot if it's mid-sentence, let the user speak
