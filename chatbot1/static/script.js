...
// =================================================================
// HOMEPAGE: click-a-card to reveal that section (nothing shows until clicked)
// =================================================================
(function () {
  const cards = document.querySelectorAll('.hero-quick-card');
  const closeBtn = document.getElementById('sectionCloseBtn');
  if (!cards.length || !closeBtn) return; // not on the homepage

  let currentSection = null;

  function openSection(id) {
    const target = document.getElementById(id);
    if (!target || !target.classList.contains('toggle-section')) return false;

    if (currentSection && currentSection !== target) {
      currentSection.classList.remove('section-open', 'in-view');
    }
    target.classList.add('section-open');
    // Trigger the fade-in immediately instead of waiting for scroll observer,
    // since the section was just made visible.
  ...
  function closeSection() {
    ...
    const quickLinks = document.querySelector('.quick-links-section');
    if (quickLinks) quickLinks.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }

  cards.forEach((card) => {
    const href = card.getAttribute('href') || '';
    if (!href.startsWith('#')) return; // e.g. the Login card, which is a real page link
    card.addEventListener('click', (e) => {
      const id = href.slice(1);
      if (openSection(id)) e.preventDefault();
    });
  });
