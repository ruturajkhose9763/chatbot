...
- Three account roles:
    main_admin  - full control (staff accounts, college info, everything)
    staff       - teacher account (profile + news/events/gallery/testimonials)
    class_rep   - student class-representative account (own class timetable + notices)
"""
...
from flask import (
    ...
    url_for, session, flash, has_request_context
)
...
def get_admissions():
    if not os.path.exists(ADMISSIONS_DATA_PATH):
...
def fmt_staff(d, branch=None, message=""):
    ...
    if branch:
        ...
        if matched_key:
            ...
            return (f"{matched_key.replace('_', ' ').title()} HOD is {info.get('hod_name', 'N/A')}.\n"
...
GREETING_REPLIES = [
    "Hello! 👋 I'm Gyan Guru, {college}'s enquiry chatbot. Ask me about courses, admission, fees, placements, facilities, news, events, or contact details.",
    "Hi there! 😊 I'm Gyan Guru from {college}. How can I help you today — admission, fees, courses, or something else?",
    "Namaste! 🙏 I'm Gyan Guru, {college}'s chatbot assistant. Ask me anything about admission, fees, courses, hostel, scholarship, and more.",
]
...
BOT_IDENTITY_REPLIES = [
    "I'm Gyan Guru 🤖, {college}'s enquiry chatbot! I can help you with admission, fees, courses, hostel, scholarship, and more.",
    "My name is Gyan Guru 🤖 — I'm here to answer your questions about {college}, like admission, fees, courses, and facilities.",
]
...
def _ai_usage_ok_and_increment():
    ...
    try:
        usage = load_json(AI_USAGE_PATH) if os.path.exists(AI_USAGE_PATH) else {}
    except Exception:
        usage = {}
...
def ai_hybrid_reply(message, data):
    ...
    try:
        ...
        prompt = (
            f"You are Gyan Guru, the friendly enquiry chatbot for {college_name}. "
...
def get_reply_with_suggestions(message):
    ...
    # Local matching (with or without context) found nothing — try the
    # optional AI hybrid backup before giving up entirely.
    ai_reply = ai_hybrid_reply(message, d)
    if ai_reply:
...
def class_rep_required(view):
    @wraps(view)
    def wrapped(*args, **kwargs):
        if not session.get("logged_in"):
            return redirect(url_for("login"))
        if session.get("role") != "class_rep":
            flash("Only class representatives can do that.")
            return redirect(url_for("admin_dashboard"))
        return view(*args, **kwargs)
    return wrapped
...
def admin_dashboard():
    ...
    my_class = None
    if current_account["role"] == "class_rep" and current_account.get("class_key"):
...
@app.route("/admin/add-class-rep", methods=["POST"])
@main_admin_required
def add_class_rep():
    accounts_data = get_accounts()
    username = request.form.get("username", "").strip()
    ...
    save_json(STAFF_ACCOUNTS_PATH, accounts_data)
    flash(f"Class Representative account '{username}' created.")
    return redirect(url_for("admin_dashboard"))
...
def update_timetable():
    ...
    if class_key in classrooms["classes"]:
        classrooms["classes"][class_key]["timetable"] = request.form.get("timetable", "").strip()
