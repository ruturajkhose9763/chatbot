...
INTERNAL_MARKS_PATH = os.path.join(BASE_DIR, "internal_marks.json")
...
def get_internal_marks():
    if not data_exists(INTERNAL_MARKS_PATH):
        save_json(INTERNAL_MARKS_PATH, {})
    return load_json(INTERNAL_MARKS_PATH)


def save_internal_marks(data):
    save_json(INTERNAL_MARKS_PATH, data)
...
def openrouter_reply(message, data, history=None):
    ...
    try:
        ...
        system_prompt = (
            ...
            f"Here is the ONLY real data you know about this college: {context_summary}. "
            f"STRICT RULES — follow these exactly:\n"
            f"1. If the student's question is about THIS college (fees, admission, staff names, "
            f"dates, seats, facilities, results, exams, hostel, scholarship, or anything "
            f"college-specific) and the exact answer is NOT clearly present in the data above, "
            f"you MUST NOT guess, estimate, or make up a plausible-sounding answer. Instead reply "
            f"exactly with: \"I don't have that exact information yet — please contact the college "
            f"office for accurate details.\" Do not add any numbers, names, or dates you're not sure of.\n"
            f"2. NEVER invent or alter any fee amount, date, seat count, percentage, or person's "
            f"name that isn't written verbatim in the data above.\n"
            f"3. If the question is NOT about the college at all (general knowledge, math, coding, "
            f"casual chat), you may answer normally using your own knowledge — that's fine, since "
            f"nothing there needs to match college data.\n"
            f"4. {length_rule}"
        )
        messages = [{"role": "system", "content": system_prompt}]
...
def _build_dashboard_context():
    ...
    my_class_marks = []
    my_class_subject = (current_account.get("subject") or "General") if current_account["role"] == "staff" else None
    if my_class is not None:
        marks_data = get_internal_marks()
        class_marks = marks_data.get(current_account.get("class_key"), {}).get(my_class_subject, {})
        for student in my_class_students:
            my_class_marks.append({"name": student, "marks": class_marks.get(student, "")})
    ...
    return dict(
        ...
        my_class_marks=my_class_marks, my_class_subject=my_class_subject,
...
def save_marks():
    account = find_account(session["username"])
    class_key = account.get("class_key")
    subject = account.get("subject") or "General"

    attendance = get_attendance()
    students = attendance["classes"].get(class_key, {}).get("students", [])

    marks_data = get_internal_marks()
    class_marks = marks_data.setdefault(class_key, {}).setdefault(subject, {})

    for student in students:
        raw = request.form.get(f"marks_{student}", "").strip()
        if raw == "":
            class_marks.pop(student, None)
            continue
        try:
            value = max(0, min(20, int(raw)))  # internal marks out of 20
            class_marks[student] = value
        except ValueError:
            pass

    save_internal_marks(marks_data)
    flash("Internal marks saved.")
    return redirect_to_dashboard()
