...
# =================================================================
# FIREBASE PHONE AUTH (optional — used for student/teacher mobile
# OTP verification, 10,000 free verifications/month on Firebase's
# Spark plan). Falls back to the older Twilio-SMS OTP flow
# automatically if this isn't configured, so nothing breaks either
# way. To enable: set FIREBASE_SERVICE_ACCOUNT_JSON (paste the full
# JSON key file content as one env var) plus FIREBASE_API_KEY,
# FIREBASE_AUTH_DOMAIN, FIREBASE_PROJECT_ID, FIREBASE_APP_ID
# (the public client config — safe to expose, not secret).
# =================================================================
_firebase_app = None
try:
    import firebase_admin
    from firebase_admin import credentials as _fb_credentials, auth as firebase_auth
    _fb_service_account_raw = os.environ.get("FIREBASE_SERVICE_ACCOUNT_JSON")
    if _fb_service_account_raw:
        _fb_cred = _fb_credentials.Certificate(json.loads(_fb_service_account_raw))
        _firebase_app = firebase_admin.initialize_app(_fb_cred)
except Exception as e:
    print("Firebase Admin SDK not configured — falling back to Twilio OTP flow:", e)
    _firebase_app = None
...
def verify_firebase_phone_token(id_token, expected_mobile):
    """Verifies a Firebase ID token (from client-side Phone Auth) and
    checks the verified phone number matches the one on file. Returns
    True/False. Any failure (bad token, mismatch, Firebase not
    configured) safely returns False rather than raising."""
    if not firebase_enabled() or not id_token:
        return False
    try:
        decoded = firebase_auth.verify_id_token(id_token)
        verified_phone = (decoded.get("phone_number") or "").strip()
        # Compare only the last 10 digits so +91XXXXXXXXXX matches XXXXXXXXXX.
        return verified_phone[-10:] == (expected_mobile or "").strip()[-10:] and bool(verified_phone)
    except Exception as e:
...
def _mongo_unavailable(e):
    """Called whenever an actual Mongo read/write fails (e.g. the cluster
    was deleted, network issue, credentials rotated). Logs it once and
    disables Mongo for the rest of this process's life so every further
    request falls back to local JSON files immediately instead of hanging
    for the connection timeout (or crashing) on every single request."""
    global _mongo_collection
    print("MongoDB read/write failed, switching to local JSON files for "
          "the rest of this session:", e)
    _mongo_collection = None
...
def load_json(path):
    if _mongo_collection is not None:
        try:
            ...
            _mongo_unavailable(e)
    # No MONGO_URI configured (e.g. local dev), or Mongo just failed above —
    # behave exactly as before, reading straight from the local JSON file.
    with open(path, "r", encoding="utf-8") as f:
        return json.load(f)
...
def get_app_settings():
    if not data_exists(APP_SETTINGS_PATH):
        save_json(APP_SETTINGS_PATH, {"voice_mode": "elevenlabs"})
    settings = load_json(APP_SETTINGS_PATH)
    settings.setdefault("voice_mode", "elevenlabs")
    return settings


def save_app_settings(data):
    save_json(APP_SETTINGS_PATH, data)
...
def get_students():
    if not data_exists(STUDENTS_DATA_PATH):
        save_json(STUDENTS_DATA_PATH, {"students": []})
...
def find_student(roll_no):
    roll = normalize_roll(roll_no)
    return next((s for s in get_students().get("students", []) if normalize_roll(s.get("roll_no")) == roll), None)


def find_student_by_name_class(name, class_key):
    name = (name or "").strip().lower()
    return next((s for s in get_students().get("students", [])
                 if str(s.get("name", "")).strip().lower() == name and s.get("class_key") == class_key), None)


def to_e164_mobile(mobile):
    """Twilio's API rejects phone numbers unless they're in E.164 format
    (e.g. +919876543210). A plain 10-digit Indian number sent as-is is the
    most common reason OTP SMS silently fails, so we normalize here."""
    digits = "".join(ch for ch in str(mobile or "") if ch.isdigit())
    if str(mobile).strip().startswith("+"):
        return "+" + digits
    if len(digits) == 11 and digits.startswith("0"):
        digits = digits[1:]  # strip old-style leading 0 (e.g. "09876543210")
    if len(digits) == 10:
        return "+91" + digits
    if len(digits) == 12 and digits.startswith("91"):
        return "+" + digits
    return "+" + digits  # last resort — better than sending a raw, unformatted number


def _send_teacher_otp_sms(mobile, otp):
    """Optional Twilio sender. Without credentials it returns False and never blocks the app."""
    sid = os.environ.get("TWILIO_ACCOUNT_SID")
    token = os.environ.get("TWILIO_AUTH_TOKEN")
    from_number = os.environ.get("TWILIO_FROM_NUMBER")
    if not (sid and token and from_number):
        print("Twilio OTP skipped: TWILIO_ACCOUNT_SID / TWILIO_AUTH_TOKEN / TWILIO_FROM_NUMBER not all set.")
        return False
    to_number = to_e164_mobile(mobile)
    try:
        import base64
        body = urlencode({
            "To": to_number, "From": from_number,
            "Body": f"College Teacher verification OTP: {otp}. Valid for 10 minutes."
        }).encode()
        req = Request(f"https://api.twilio.com/2010-04-01/Accounts/{sid}/Messages.json", data=body, method="POST")
        req.add_header("Authorization", "Basic " + base64.b64encode(f"{sid}:{token}".encode()).decode())
        req.add_header("Content-Type", "application/x-www-form-urlencoded")
        with urlopen(req, timeout=8) as resp:
            ok = 200 <= resp.status < 300
            if not ok:
                print(f"Twilio OTP SMS to {to_number} failed: HTTP {resp.status} — {resp.read()}")
            return ok
    except Exception as exc:
        # Twilio's real error message (e.g. "unverified number" on trial
        # accounts, bad From number, insufficient balance) ends up here —
        # check the Render logs for this line if OTPs still don't arrive.
        print(f"Twilio OTP SMS to {to_number} failed:", exc)
        return False


def _new_notification(title, message, note_type="system"):
    return {
        "id": secrets.token_hex(8), "type": note_type, "title": title,
        "message": message, "date": datetime.now().strftime("%Y-%m-%d %H:%M"), "read": False
    }
...
BRANCH_ALIASES = {
    ...
    "civil": ["civil", "सिव्हिल"],
    "mechanical": ["mechanical", "mech", "मेकॅनिकल"],
    "electrical": ["electrical", "eee", "इलेक्ट्रिकल"],
}

YEAR_ALIASES = {
    "first": ["1st year", "first year", "pahile varsh", "pahla saal", "पहिले वर्ष"],
    "second": ["2nd year", "second year", "direct second", "dsy", "lateral", "दुसरे वर्ष"],
    "third": ["3rd year", "third year", "tisra saal", "tisre varsh", "तिसरे वर्ष"],
}
...
SCHOLARSHIP_WORDS = ["scholarship", "scholarships", "chatravrutti", "chatrawrutti", "mahadbt",
                      "freeship", "tfws", "shishyavrutti", "शिष्यवृत्ती", "स्कॉलरशिप"]
...
DOCUMENTS_WORDS = ["document", "documents", "kagaz", "kagajpatra", "certificate", "certificates",
                    "papers needed", "paperwork", "कागदपत्रे"]
...
def fmt_courses(d, branch=None, message=""):
    ...
    if branch:
        ...
        c = courses[0]
        dsy = c.get('direct_second_year_seats', 'N/A')
        return (f"Great choice! {c['name']} is a {c['duration']} Diploma course. "
                f"We currently have {c['seats']} seats for 1st year, and {dsy} seats for Direct 2nd Year entry. 😊")
    lines = [f"Sure! Here are the Diploma courses we offer at {d.get('college_name', 'our college')} 🎓\n"]
    for c in courses:
        dsy = c.get('direct_second_year_seats', 'N/A')
        lines.append(f"• {c['name']} ({c['duration']}) — {c['seats']} seats, {dsy} Direct 2nd Year seats")
    return "\n".join(lines)
...
def fmt_seats(d, branch=None, message=""):
    ...
    if branch:
        ...
        c = courses[0]
        dsy = c.get('direct_second_year_seats', 'N/A')
        return (f"In {c['name']}, we currently have {c['seats']} seats for 1st year, "
                f"and {dsy} seats available under Direct 2nd Year admission. 😊")
    lines = ["Here's how the seats are divided across branches:\n"]
    for c in courses:
        dsy = c.get('direct_second_year_seats', 'N/A')
        lines.append(f"• {c['name']}: {c['seats']} seats (1st year), {dsy} seats (Direct 2nd Year)")
    return "\n".join(lines)
...
def fmt_admission(d, branch=None, message=""):
    ...
    if mentions_any(msg, ["eligibility", "patrata", "पात्रता", "eligible"]):
        return f"For eligibility, here's what's needed: {a.get('eligibility', 'not specified right now')}."

    if mentions_any(msg, ["minimum marks", "kitne percent", "kitna percentage", "% marks"]):
        return f"You'll need a minimum of {a.get('minimum_marks', 'the marks specified by the eligibility criteria')} to be eligible. 😊"

    if mentions_any(msg, ["how to apply", "kaise apply", "apply kaise", "kaise le", "kaise admission le"]):
        return f"Here's how you can apply: {a.get('how_to_apply', 'please contact the college office for the application steps')}."

    if mentions_any(msg, ["entrance", "cap", "merit"]):
        return f"The entrance/admission process works like this: {a.get('entrance_exam', 'please contact the office for details')}."

    if mentions_any(msg, ["institute code", "college code"]):
        return f"Our Institute Code is {a.get('institute_code', 'not available right now')}."

    # No specific sub-topic asked -> give the full overview (reasonable default)
    branch_txt = f" for {branch.title()} Engineering" if branch else ""
    return (
        f"Happy to help with admission{branch_txt}! 😊 Here's a quick overview:\n\n"
        f"• Eligibility: {a.get('eligibility', 'N/A')}\n"
        f"• Minimum marks required: {a.get('minimum_marks', 'N/A')}\n"
        f"• Entrance/admission process: {a.get('entrance_exam', 'N/A')}\n"
        f"• How to apply: {a.get('how_to_apply', 'N/A')}\n"
        f"• Institute Code: {a.get('institute_code', 'N/A')}\n\n"
        f"Want me to go deeper into any one of these?"
    )
...
def fmt_fees(d, branch=None, message=""):
    ...
    if mentions_any(msg, ["second year", "2nd year", "dsy", "lateral", "direct second"]):
        lines = ["Here's the Direct Second Year fee breakdown 💰"]
        for k, v in dsy.items():
            lines.append(f"• {k.replace('_', ' / ')}: {v}")
        return "\n".join(lines)

    if mentions_any(msg, ["first year", "1st year"]):
        lines = ["Here's the First Year fee breakdown 💰"]
        for k, v in fy.items():
            lines.append(f"• {k.replace('_', ' / ')}: {v}")
        return "\n".join(lines)

    lines = ["Sure, here's the fee structure for our Diploma courses 💰", "\nFirst Year:"]
    for k, v in fy.items():
        lines.append(f"• {k.replace('_', ' / ')}: {v}")
    lines.append("\nDirect Second Year:")
    for k, v in dsy.items():
...
def fmt_facilities(d, branch=None, message=""):
    lines = ["Here's what we have on campus 🏫"]
...
def fmt_placements(d, branch=None, message=""):
    ...
    if mentions_any(msg, ["company", "companies", "recruiter", "recruiters"]) and not mentions_any(msg, ["package", "salary", "rate", "%"]):
        lines = ["Some of the top recruiters who visit our campus are:"]
        lines += [f"• {c}" for c in p.get("top_recruiters", [])]
        return "\n".join(lines)

    if mentions_any(msg, ["package", "salary", "lpa"]) and not mentions_any(msg, ["company", "companies"]):
        return (f"On the package front — our average package is {p.get('average_package', 'N/A')}, "
                f"the highest offered so far is {p.get('highest_package', 'N/A')}, "
                f"and the starting package is around {p.get('starting_package', 'N/A')}. 💼")

    lines = [
        "Good news on placements! 🎉",
        f"We currently have a placement rate of {p.get('placement_rate', 'N/A')}, with an average package of "
        f"{p.get('average_package', 'N/A')} (highest going up to {p.get('highest_package', 'N/A')}, "
        f"starting around {p.get('starting_package', 'N/A')}).",
...
def fmt_contact(d, branch=None, message=""):
    ...
    return (
        ...
        f"We're located at {c.get('address', 'the address is not available right now')}. "
        f"You can call us on {c.get('phone', 'N/A')} or email {c.get('email', 'N/A')}. "
        f"Our Institute Code is {c.get('institute_code', 'N/A')}."
    )
...
def fmt_timings(d, branch=None, message=""):
    t = d.get("timings", {})
    return (
        f"Our college runs from {t.get('college_hours', 'N/A')}, and the office is open {t.get('office_hours', 'N/A')}. "
        f"On Saturdays, it's {t.get('saturday', 'N/A')}."
    )
...
def fmt_staff(d, branch=None, message=""):
    ...
    if branch:
        ...
        if matched_key:
            ...
            dept_title = matched_key.replace('_', ' ').title()
            contact = info.get('contact_number')
            email = info.get('email')
            reply = f"The HOD of {dept_title} is {info.get('hod_name', 'N/A')}."
            if contact and contact.lower() not in ('n/a', 'not available', ''):
                reply += f" You can reach them at {contact}."
            if email and email.lower() not in ('n/a', 'not available', ''):
                reply += f" Email: {email}."
            committee_name = next((v for k, v in committee_hods.items() if branch in k.lower()), None)
            if committee_name:
                reply += f" (Note: in the college's official committee records, {committee_name} is listed as the committee-designated HOD for this department.)"
            return reply
...
def fmt_why_choose_us(d, branch=None, message=""):
    p = d.get("placements", {})
    facilities = d.get("facilities", [])
    lines = [
        f"Great question! 😊 Here's why {d.get('college_name', 'our college')} could be a great choice for you:\n",
        f"🎓 {p.get('placement_rate', 'Strong')} placement rate — recent students placed with packages up to {p.get('highest_package', 'good companies')}.",
        f"🏢 Recruiters include: {', '.join(p.get('top_recruiters', [])[:5]) or 'reputed companies'}.",
    ]
    if facilities:
        lines.append(f"🏫 Facilities: {', '.join(facilities[:5])}.")
    about_text = d.get('about', '')
    if len(about_text) > 220:
        cut = about_text.rfind('.', 0, 220)
        about_text = about_text[:cut + 1] if cut > 0 else about_text[:220] + "..."
    lines.append(f"\n{about_text}")
    lines.append("\nWant details on a specific branch, fees, or the admission process? Just ask! 👍")
    return "\n".join(lines)
...
def fmt_news(_d, branch=None, message=""):
    ...
    notices = content.get("notices", [])
    lines = []

    if notices:
        lines.append("📌 Notice Board:")
        for n in notices[:8]:
            tag = "🔴 URGENT — " if n.get("priority") == "urgent" else ""
            lines.append(f"• {tag}{n['text']} ({n['date']})")

    if news:
        if lines:
            lines.append("")
        lines.append("📰 Latest News:")
        for n in sorted(news, key=lambda x: x["date"], reverse=True)[:6]:
            lines.append(f"• [{n['date']}] {n['title']} — {n['description']}")

    if not lines:
        return "No news or notices right now. Check back soon! 📰"
    return "\n".join(lines)
...
def fmt_scholarship(d, branch=None, message=""):
    ...
    if mentions_any(msg, ["apply", "how to apply", "kaise apply", "kaise", "mahadbt", "portal"]):
        return f"To apply for scholarship: {s.get('how_to_apply', 'please contact the college office for the steps')}."

    if mentions_any(msg, ["eligib", "eligible", "kaun", "patrata"]):
        return f"Scholarship eligibility: {s.get('eligibility', 'please contact the college office for details')}."

    lines = ["Here's the scholarship information 🎓"]
    note = s.get("note", "")
    if note:
        lines.append(f"\n{note}")
    lines.append("\nThe scholarships available include:")
    lines += [f"• {t}" for t in s.get("types", [])]
    lines.append(f"\nTo apply: {s.get('how_to_apply', 'N/A')}")
    return "\n".join(lines)
...
def fmt_hostel(d, branch=None, message=""):
    ...
    if mentions_any(msg, ["fee", "fees", "paisa", "paise", "cost"]):
        return f"Hostel fees: {h.get('fees_note', 'please contact the college office for the exact amount')}."

    if mentions_any(msg, ["apply", "admission", "kaise le"]):
        return f"To get hostel admission: {h.get('how_to_apply', 'please contact the college office')}."
    ...
    fees_note = h.get('fees_note', '')
    if fees_note:
        lines.append(f"\n{fees_note}")
    lines.append(f"To apply: {h.get('how_to_apply', 'N/A')}")
    return "\n".join(lines)


def fmt_exam_schedule(d, branch=None, message=""):
    e = d.get("exam_schedule", {})
    lines = ["Here's the exam information 📝"]
    note = e.get("note", "")
    if note:
        lines.append(f"\n{note}")
    lines.append("\nExams are conducted as:")
    lines += [f"• {t}" for t in e.get("types", [])]
    lines.append(f"\nTo check exact dates: {e.get('how_to_check', 'N/A')}")
...
INTENTS = [
    {"name": "greeting", "keywords": ["hi", "hello", "hey", "namaskar", "नमस्कार",
                                        "good morning", "good evening"], "answer": None},
    {"name": "about", "keywords": ["about", "about college", "college baddal", "college kasa aahe",
                                     "कॉलेज कसे आहे", "college mahiti", "history"], "answer": fmt_about},
    {"name": "why_choose_us", "keywords": [
        "why choose", "why should i join", "why should i take admission", "kyun admission", "kyu admission",
        "kyun le admission", "is college me kya khaas", "kya khaas hai is college me", "best kyu hai",
        "why this college", "college achha hai kya", "should i join this college", "worth joining",
        "college che fayde", "college chi khasiyat", "काय खास आहे", "प्रवेश का घ्यावा"],
     "answer": fmt_why_choose_us},
    ...
    {"name": "classroom", "keywords": ["timetable", "time table", "class schedule", "classroom",
                     "वेळापत्रक", "class chi mahiti"], "answer": fmt_classroom},
    ...
    {"name": "seats", "keywords": ["seat", "seats", "kiti seat", "intake", "student", "students", "kiti student",
                     "kiti students", "how many student", "how many students", "kiti mule",
                     "सीट", "जागा", "किती जागा", "kiti jaga"],
     "answer": fmt_seats},
    {"name": "admission", "keywords": ["admission", "admison", "eligibility", "apply", "entrance", "admission kase",
                     "admission kasa ghyaycha", "admission process", "cap", "merit",
                     "प्रवेश", "प्रवेश कसा घ्यायचा", "पात्रता", "patrata"], "answer": fmt_admission},
    {"name": "documents", "keywords": ["document", "documents", "kagaz", "kagajpatra", "certificate", "papers needed",
                     "कागदपत्रे"], "answer": fmt_documents},
    {"name": "fees", "keywords": ["fee", "fees", "paisa", "paise", "kharcha", "cost", "fee kiti", "total fees",
                     "फीस", "किती पैसे", "kiti paise"], "answer": fmt_fees},
    {"name": "facilities", "keywords": ["facility", "facilities", "lab", "library", "hostel", "canteen",
                     "sports", "wifi", "computer lab", "workshop", "cricket", "football", "ground",
                     "playground", "gym", "gymkhana", "volleyball", "kabaddi", "khel", "khelte",
                     "सुविधा", "सोयीसुविधा", "वसतिगृह", "ग्रंथालय", "लायब्ररी"], "answer": fmt_facilities},
    {"name": "placements", "keywords": ["placement", "placements", "job", "jobs", "package", "salary",
                     "company", "companies", "naukri", "recruit",
                     "नोकरी", "प्लेसमेंट", "प्लेसमेंट कसे आहे", "पगार", "salary kiti"], "answer": fmt_placements},
    {"name": "contact", "keywords": ["contact", "phone", "number", "email", "address", "pata", "location", "kuthe aahe",
                     "संपर्क", "फोन नंबर", "पत्ता", "कुठे आहे"], "answer": fmt_contact},
    {"name": "timings", "keywords": ["timing", "timings", "time", "samay", "hours", "kadhi suru", "college time",
                     "वेळ", "वेळापत्रक", "कधी सुरू"], "answer": fmt_timings},
    ...
    {"name": "committees", "keywords": ["committee", "committees", "anti ragging", "grievance", "iqac",
                     "governing body", "समिती", "कमिटी"], "answer": fmt_committees},
    ...
    {"name": "news", "keywords": ["news", "notice", "announcement", "khabar", "बातम्या", "सूचना"],
     "answer": fmt_news},
    {"name": "events", "keywords": ["event", "events", "function", "karyakram", "कार्यक्रम", "इव्हेंट", "उपक्रम"],
     "answer": fmt_events},
    {"name": "scholarship", "keywords": ["scholarship", "scholarships", "chatravrutti", "chatrawrutti",
                     "mahadbt", "freeship", "tfws", "shishyavrutti",
                     "शिष्यवृत्ती", "स्कॉलरशिप"], "answer": fmt_scholarship},
    ...
    {"name": "exam_schedule", "keywords": ["exam schedule", "exam timetable", "exam date", "exam dates",
                     "msbte exam", "pariksha", "kadhi ahe exam", "exam kadhi ahe",
                     "परीक्षा वेळापत्रक", "परीक्षेचे वेळापत्रक", "परीक्षा कधी"], "answer": fmt_exam_schedule},
    {"name": "thanks", "keywords": ["thanks", "thank you", "thanku", "thnx", "dhanyawad",
                     "धन्यवाद", "आभारी आहे"], "answer": None},
    {"name": "goodbye", "keywords": ["bye", "bye bye", "goodbye", "tata", "punha bhetu",
                     "chalta hoon", "nighto"], "answer": None},
    {"name": "bot_identity", "keywords": ["your name", "tujha naav kay", "tujha naav kay ahe", "tu kon ahes",
                     "who are you", "bot cha naav", "tu kon ahes"], "answer": None},
]

FALLBACK = (
    "Hmm, I'm not sure about that one 🤔 / Maaf kara, hi mahiti sadhya upalabdha nahi. "
...
EMOTION_WORDS = {
    ...
    "gussa", "angry", "frustrated", "समजत नाही", "लवकर",
}
...
def get_best_intent(message):
    ...
    if fuzzy_mentions_any(msg, DOCUMENTS_WORDS, threshold=0.82):
        return next(i for i in INTENTS if i["name"] == "documents"), 99
    ...
    for intent in INTENTS:
        if intent["name"] == "admission_contact":
            continue
        # Smalltalk words (hi/hello/thanks/bye) are only meaningful when the
        # whole message is short/casual. A long message that just happens to
        # contain one of these words (e.g. "hello world program in java")
        # should never be treated as a greeting.
        if intent["name"] in SMALLTALK_INTENTS and len(msg_words) > 5:
            continue
        score = 0
...
REFINEMENT_KEYWORDS = {
    ...
    "documents": ["first year", "1st year", "second year", "2nd year", "dsy",
                  "lateral", "direct second"],
...
CODE_REQUEST_PATTERN = re.compile(
    r'\b(python|java(script)?|c\+\+|c#|programming language|coding|'
    r'algorithm|function likh|code likh|program likh|likh do|'
    r'write.*(program|code|script|function)|program (do|de|bana)|'
    r'code (do|de|likho)|debug|syntax error)\b',
    re.IGNORECASE,
)


def is_code_request(message):
    """True if the message is clearly asking for code/a program, as opposed
    to an academic 'program/course' — used so the bot (a) doesn't misfire
    the local 'courses' intent just because both use the word "program",
    and (b) doesn't cram real code into an artificially short word limit."""
    return bool(CODE_REQUEST_PATTERN.search(message or ""))
...
def ai_hybrid_reply(message, data, history=None):
    ...
    try:
        ...
        length_rule = (
            "The student is asking for code — go ahead and write a complete, correctly "
            "formatted code snippet (with proper indentation and a short explanation)."
            if code_mode else
        ...
        system_prompt = (
            ...
            "Prefer answering the student's question using the JSON college data given below "
            "whenever it's relevant. If the question isn't about the college at all (general "
            "knowledge, casual chat, a quick calculation, a coding question, etc.), it's fine to "
            "answer it yourself using your own knowledge — just never invent or guess specific "
            "college facts (fees, seats, staff names, dates) that aren't in the data below; for "
            f"those, say you don't have that information and suggest contacting the college office. "
            f"{length_rule} Use the earlier conversation turns (if any) for context, "
        ...
        resp = requests.post(
            ...
            json={
                "contents": contents,
                "generationConfig": {"maxOutputTokens": 800 if code_mode else 300},
            },
            timeout=8 if not code_mode else 15,
        )
...
# Topic intents worth trying to classify when keyword-matching fails
# (small-talk intents like greeting/thanks/bye/bot_identity are excluded —
# those keyword-match reliably enough on their own and don't need this).
CLASSIFIABLE_INTENTS = [
    "about", "why_choose_us", "classroom", "courses", "seats", "admission",
    "documents", "fees", "facilities", "placements", "contact", "timings",
    "staff", "committees", "institution_details", "news", "events",
    "scholarship", "hostel", "exam_schedule",
]


def ai_classify_intent(message, history=None):
    """When the local keyword matcher can't tell what topic a question is
    about (odd phrasing, indirect wording, etc.), ask a fast/free OpenRouter
    model to pick the single closest matching topic from our real list of
    intents. This makes the bot "understand" more varied phrasing, while
    the actual answer still always comes from the real formatter/data —
    the AI never invents the answer itself here, only picks which real
    answer to show. Returns an intent name string, or None."""
    api_key = os.environ.get("OPENROUTER_API_KEY")
    if not api_key:
        return None
    try:
        import requests
        topic_list = ", ".join(CLASSIFIABLE_INTENTS)
        system_prompt = (
            "A student typed a message to a college enquiry chatbot. Pick the ONE topic "
            f"from this exact list that their message is most likely about: {topic_list}. "
            "Reply with ONLY that single topic word, nothing else. "
            "If it doesn't clearly match any of these topics (e.g. general chit-chat, a "
            "totally unrelated question, or too vague to tell), reply with exactly: none"
        )
        ...
        response = requests.post(
            ...
            headers={
                ...
                "HTTP-Referer": os.environ.get("SITE_URL", "https://sedcoe.onrender.com"),
                "X-Title": "College Chatbot Intent Classifier",
            },
            json={
        ...
        if response.status_code == 200:
            ...
            if choices:
                guess = choices[0].get("message", {}).get("content", "").strip().lower()
                guess = re.sub(r"[^a-z_]", "", guess)
                if guess in CLASSIFIABLE_INTENTS:
                    return guess
        else:
            print(f"Intent classifier error: {response.status_code} - {response.text[:200]}")
    except Exception as e:
        print(f"Intent classifier failed: {e}")
    return None


def openrouter_reply(message, data, history=None):
    api_key = os.environ.get("OPENROUTER_API_KEY")
    if not api_key:
        return None
    try:
        import requests
        college_name = data.get("college_name", "the college")
        context_summary = json.dumps(data, ensure_ascii=False)[:4000]
        code_mode = is_code_request(message)
        length_rule = (
            "The student is asking for code — go ahead and write a complete, correctly "
            "formatted code snippet (with proper indentation and a short explanation)."
            if code_mode else
            "Keep reply short (under 80 words), friendly, in Hinglish/English."
        )
        system_prompt = (
            f"You are College AI Assistant for {college_name}. "
            f"Prefer answering using this college data whenever it's relevant: {context_summary}. "
            f"If the student's question isn't about the college at all (e.g. general knowledge, "
            f"a math question, a coding question, casual chat), it's fine to answer it yourself "
            f"using your own knowledge — just don't invent or guess any college-specific facts "
            f"(fees, seats, staff names, dates) that aren't in the data above. "
            f"{length_rule}"
        )
        messages = [{"role": "system", "content": system_prompt}]
        for turn in (history or [])[-(6 * 2):]:
            role = "assistant" if turn["role"] == "model" else "user"
            messages.append({"role": role, "content": turn["text"]})
        messages.append({"role": "user", "content": message})
        response = requests.post(
            ...
            headers={
                ...
                "Content-Type": "application/json",
                "HTTP-Referer": os.environ.get("SITE_URL", "https://sedcoe.onrender.com"),
                "X-Title": f"{college_name} Chatbot",
            },
            json={
                "model": "openrouter/free",
                "messages": messages,
                "max_tokens": 800 if code_mode else 300,
                "temperature": 0.7,
            },
            timeout=15 if code_mode else 10,
        )
        if response.status_code == 200:
            result = response.json()
            choices = result.get("choices", [])
            if choices:
                ...
                return reply_text.strip() or None
        else:
...
def get_reply_with_suggestions(message):
    if not message.strip():
        return "Please type a question. / Krupaya tumcha prashna type kara.", []

    d = get_data()
    cleaned_msg = clean(message)
    emotional = detect_emotion(cleaned_msg)

    # BUG FIX: code/programming requests must NEVER go through local keyword
    # matching at all — not just to avoid "program" hijacking "courses", but
    # because plenty of ordinary coding requests collide with totally
    # unrelated keywords too (e.g. "hello world program" contains the exact
    # word "hello", which is a greeting keyword, and would otherwise return
    # a greeting instead of code). So this check runs first, before
    # get_best_intent is even called, and skips straight to the AI replies.
    if is_code_request(message):
        ...
        return FALLBACK, ["Courses konse hai?", "Fees kitni hai?", "Admission kaise le?"]
    ...
    if intent and score > 0 and score >= 10:
    ...
    if last_intent_name and len(cleaned_msg.split()) <= 6:
        ...
        # NOTE: "documents" doesn't actually vary by branch (fmt_documents
        # ignores it), so a bare branch mention alone must NOT count as
        # "still talking about documents" — that used to wrongly re-answer
        # unrelated follow-ups (e.g. "computer branch me staff kaun hai?")
        # with the old documents list. It only continues on an explicit
        # refinement like "1st year" / "2nd year".
        branch_counts_as_relevant = bool(branch) and last_intent_name != "documents"
        relevant = branch_counts_as_relevant or fuzzy_mentions_any(
    ...
    # Local matching (with or without context) found nothing — before
    # giving up on "understanding" the question, try asking a fast AI model
    # to classify which real topic this is about (handles odd phrasing that
    # doesn't contain our exact keywords). The actual answer still always
    # comes from our real data/formatter function, never invented by the AI.
    # BUG FIX: requests like "Python ka program do" / "write me a program"
    # were getting misclassified as the "courses" intent, because the word
    # "program" also means "academic program/course" — so the bot replied
    # with course info instead of writing code. If the message clearly asks
    # for code/a program in a programming sense, skip local intent
    # classification entirely and go straight to the general AI reply.
    guessed_intent_name = None if is_code_request(message) else ai_classify_intent(message, history=_history_get())
    if guessed_intent_name:
        guessed_intent = next((i for i in INTENTS if i["name"] == guessed_intent_name), None)
        if guessed_intent:
            try:
                is_branch_aware = guessed_intent_name in BRANCH_AWARE_INTENTS
                branch = detect_branch(message) if is_branch_aware else None
                reply = (guessed_intent["answer"](d, branch=branch, message=message)
                         if is_branch_aware
                         else guessed_intent["answer"](d, message=message))
                if reply:
                    if emotional:
                        reply = random.choice(EMOTION_OPENERS) + reply
                    log_chat_question(guessed_intent_name, message)
                    _session_set("last_intent", guessed_intent_name)
                    _history_append(message, reply)
                    return reply, FOLLOWUPS.get(guessed_intent_name, [])
            except Exception:
                pass

    # Still nothing understood — check if an admin has already taught the
    # bot an answer for a similar previously-unanswered question.
    custom_answer = match_custom_qa(cleaned_msg, cleaned_msg.split())
    if custom_answer:
        if emotional:
...
def home():
    ...
    return render_template(
        "index.html", college=data, news=news, events=events, gallery=gallery, notices=notices,
...
def full_chat():
    data = get_data()
    voice_mode = get_app_settings().get("voice_mode", "elevenlabs")
    return render_template("chat.html", college=data, voice_mode=voice_mode)
...
def chat():
    ...
    reply_lang = (body.get("lang") or "en").strip()  # "en" or "mr" — same selector as voice
...
def translate_text(text, target_lang):
    """Translate `text` into target_lang ('mr') using Gemini.
...
def translate_endpoint():
    ...
    target_lang = (body.get("lang") or "").strip()  # "mr" or "en"
    if not text or target_lang not in ("mr",):
...
# ---- Voice output ----
# Primary: Microsoft Edge's online neural voices via the free, keyless
# "edge-tts" library — no account, no quota, good Hindi/Marathi/English voices.
# Secondary (optional): ElevenLabs, only used if ELEVENLABS_API_KEY is set
# AND the account has a paid plan (needed for these specific voices).
#
# Each of the 3 "personas" the college picked maps to a natural-sounding
# Edge voice per language, so switching the reply-language dropdown also
# switches to the right-language voice automatically.
EDGE_TTS_VOICE_MAP = {
    ("manav", "en"): "en-IN-PrabhatNeural",
    ("manav", "mr"): "mr-IN-ManoharNeural",
    ("roopa", "en"): "en-IN-NeerjaNeural",
    ("roopa", "mr"): "mr-IN-AarohiNeural",
    ("tripti", "en"): "en-IN-NeerjaNeural",
    ("tripti", "mr"): "mr-IN-AarohiNeural",
}

ELEVENLABS_VOICE_ID = os.environ.get("ELEVENLABS_VOICE_ID", "6MoEUz34rbRrmmyxgRm4")  # "Manav"
ELEVENLABS_VOICES = {
    "manav": "6MoEUz34rbRrmmyxgRm4",
    "roopa": "XdxOBLRN2UCpaYs8XVA7",
    "tripti": "MwUMLXurEzSN7bIfIdXF",
}


def synthesize_edge_tts(text, voice_name):
    import asyncio
    import edge_tts
...
def tts_endpoint():
    """Turns chatbot reply text into natural-sounding speech.
    Tries the free Edge TTS voices first (no key, no quota). Returns raw
    MP3 bytes on success. On any failure it returns 204 with no body — the
    frontend then quietly falls back to the browser's own built-in voice,
    so the chatbot's voice mode never breaks. A short reason is included
    in the X-TTS-Error response header for debugging (visible in the
    browser's Network tab under this request's Response Headers)."""
    body = request.get_json() or {}
    text = (body.get("text") or "").strip()
    if not text:
        return Response("", status=204, headers={"X-TTS-Error": "empty-text"})
    text = text[:800]

    voice_key = (body.get("voice") or "manav").strip().lower()
    lang = (body.get("lang") or "en").strip().lower()
    if lang not in ("en", "mr"):
        lang = "en"

    # ---- 1) Edge TTS (free, default) ----
    edge_voice = EDGE_TTS_VOICE_MAP.get((voice_key, lang), "en-IN-NeerjaNeural")
    try:
        audio_bytes = synthesize_edge_tts(text, edge_voice)
        if audio_bytes:
            return Response(audio_bytes, mimetype="audio/mpeg")
        print("Edge TTS returned no audio data")
    except Exception as e:
        print(f"Edge TTS failed: {e}")

    # ---- 2) ElevenLabs (only if a key is set — e.g. a paid plan was added later) ----
    api_key = os.environ.get("ELEVENLABS_API_KEY")
    if api_key:
        voice_id = ELEVENLABS_VOICES.get(voice_key, ELEVENLABS_VOICE_ID)
        try:
            import requests
            resp = requests.post(
            ...
            print(f"ElevenLabs TTS error: {resp.status_code} - {resp.text[:300]}")
        except Exception as e:
            print(f"ElevenLabs TTS failed: {e}")

    return Response("", status=204, headers={"X-TTS-Error": "edge-tts-and-elevenlabs-failed"})
...
def _build_dashboard_context():
    ...
    if my_class is not None:
        ...
        class_att = attendance_data["classes"].get(class_key, {"students": [], "records": {}})
...
def student_dashboard():
    ...
    my_class = get_classrooms()["classes"].get(student.get("class_key")) or {}
    return render_template("student.html", college=get_data(), student=student,
                           ...
                           timetable_rows=my_class.get("timetable_rows", []),
                           class_notices=my_class.get("notices", [])[:10])
...
def student_verify():
    ...
    return render_template("student_verify.html", college=get_data(), roll_no=roll_no, mobile=student.get("mobile", ""),
                           firebase_enabled=firebase_enabled(), firebase_config=FIREBASE_CLIENT_CONFIG)


@app.route("/student/verify-firebase", methods=["POST"])
def student_verify_firebase():
    """Verifies a phone number using a Firebase Phone-Auth ID token sent
    from the browser (client-side flow — 10,000 free verifications/month
    on Firebase, instead of Twilio's 5-number trial limit)."""
    body = request.get_json() or {}
    roll_no = normalize_roll(body.get("roll_no"))
    id_token = body.get("id_token")
    student = find_student(roll_no)
    if not student:
        return jsonify({"ok": False, "error": "Student account not found."}), 404
    if verify_firebase_phone_token(id_token, student.get("mobile", "")):
        data = get_students()
        target = find_student(roll_no)
        target["mobile_verified"] = True
        target["otp_hash"] = None
        ...
        save_students(data)
        return jsonify({"ok": True})
    return jsonify({"ok": False, "error": "Could not verify this phone number."}), 400
...
@app.route("/teacher/verify-firebase", methods=["POST"])
def teacher_verify_firebase():
    """Same Firebase Phone-Auth verification as the student flow, for teacher accounts."""
    body = request.get_json() or {}
    ...
    if verify_firebase_phone_token(id_token, account.get("mobile", "")):
        account["mobile_verified"] = True
        account["otp_hash"] = None
        account["otp_expires"] = None
        save_json(STAFF_ACCOUNTS_PATH, get_accounts())
...
@app.route("/admin/set-voice-mode", methods=["POST"])
@main_admin_required
def set_voice_mode():
    mode = request.form.get("voice_mode", "elevenlabs")
    ...
    if mode == "browser":
        flash("Chatbot voice switched to the backup (browser) voice.")
    else:
        flash("Chatbot voice switched back to the premium ElevenLabs voice.")
    return redirect_to_dashboard()
...
def add_staff():
    ...
    if find_account(username):
        ...
        return redirect_to_dashboard()
    if not username or len(password) < 6 or not full_name or not mobile:
        flash("Teacher name, username, mobile number and a password of at least 6 characters are required.")
        return redirect_to_dashboard()
    new_id = max([a["id"] for a in accounts_data["accounts"]], default=0) + 1
    accounts_data["accounts"].append({
        "id": new_id, "username": username, "password_hash": generate_password_hash(password),
        "full_name": full_name, "role": "staff", "department": request.form.get("department", "").strip(),
        "subject": request.form.get("subject", "").strip(), "class_key": request.form.get("class_key") or None,
        "designation": None, "dp_filename": None, "mobile": mobile, "mobile_verified": not bool(mobile),
        "otp_hash": None, "otp_expires": None
    })
    save_json(STAFF_ACCOUNTS_PATH, accounts_data)
    if mobile:
        otp=f"{secrets.randbelow(1000000):06d}"
        accounts_data["accounts"][-1]["otp_hash"]=generate_password_hash(otp)
        accounts_data["accounts"][-1]["otp_expires"]=(datetime.now()+timedelta(minutes=10)).timestamp()
        save_json(STAFF_ACCOUNTS_PATH, accounts_data)
        if _send_teacher_otp_sms(mobile, otp):
            flash(f"Teacher account '{username}' created. Verification OTP sent to the registered mobile number.")
        else:
            flash(f"Teacher account '{username}' created. Mobile verification is pending; configure Twilio to send the OTP by SMS.")
            print(f"Teacher OTP for {username}: {otp}")
    else:
        flash(f"Teacher account '{username}' created. No mobile number was provided, so mobile verification is not required.")
    return redirect_to_dashboard()
...
@app.route("/admin/ai-write-description", methods=["POST"])
@login_required
def ai_write_description():
    """Admin/teacher types a short title or a few keywords, this expands it
    into a proper 1-2 sentence News/Event description using OpenRouter's
    free model. Returns JSON {description: "..."} on success, or
    {description: None} if the AI isn't available — the admin just types
    it themselves in that case, nothing breaks."""
    body = request.get_json() or {}
    title = (body.get("title") or "").strip()
    kind = (body.get("kind") or "news").strip()
    if not title:
        return jsonify({"description": None})

    api_key = os.environ.get("OPENROUTER_API_KEY")
    if not api_key:
        return jsonify({"description": None, "debug_error": "no-openrouter-api-key-set-on-server"})

    try:
        import requests
        college_name = get_data().get("college_name", "the college")
        what = "news announcement" if kind == "news" else "college event"
        system_prompt = (
            f"You write short, clear {what} descriptions for {college_name}'s website. "
            "Given just a title/topic from the admin, write ONE friendly, professional "
            "description (1-2 sentences, under 40 words) suitable for students to read. "
            "Reply with ONLY the description text, no quotes, no extra commentary."
        )
        response = requests.post(
            url="https://openrouter.ai/api/v1/chat/completions",
            headers={
                ...
                "Content-Type": "application/json",
                "HTTP-Referer": os.environ.get("SITE_URL", "https://sedcoe.onrender.com"),
                "X-Title": f"{college_name} Admin Content Helper",
            },
            json={
                "model": "openrouter/free",
                "messages": [
        ...
        if response.status_code == 200:
            ...
            choices = result.get("choices", [])
            if choices:
                desc = choices[0].get("message", {}).get("content", "").strip().strip('"')
                if desc:
                    return jsonify({"description": desc})
            return jsonify({"description": None, "debug_error": f"empty-response: {response.text[:200]}"})
        else:
            print(f"AI description error: {response.status_code} - {response.text[:200]}")
            return jsonify({"description": None, "debug_error": f"openrouter-{response.status_code}: {response.text[:200]}"})
    except Exception as e:
        print(f"AI description failed: {e}")
        return jsonify({"description": None, "debug_error": f"exception: {str(e)[:200]}"})
...
def add_notice():
    content = get_content()
    notices = content.setdefault("notices", [])
    new_id = (max([n["id"] for n in notices], default=0)) + 1
    notices.insert(0, {
        "id": new_id,
        "text": request.form.get("text", "").strip(),
        "priority": request.form.get("priority", "normal"),  # "urgent" or "normal"
...
def delete_item(kind, item_id):
    content = get_content()
    if kind in ("news", "events", "testimonials", "downloads", "notices"):
