...
# ElevenLabs voice IDs — pleasant, natural-sounding narrator voices that work
# reasonably well across English/Hindi/Marathi text.
ELEVENLABS_VOICE_ID = os.environ.get("ELEVENLABS_VOICE_ID", "6MoEUz34rbRrmmyxgRm4")  # "Manav" — charming, husky, warm

# The 3 voices the college chose from the ElevenLabs Voice Library. The
# frontend sends one of these short keys; anything unrecognised (or missing)
# falls back to "manav".
ELEVENLABS_VOICES = {
    "manav": "6MoEUz34rbRrmmyxgRm4",   # Manav — Charming, husky and warm
    "roopa": "XdxOBLRN2UCpaYs8XVA7",   # Roopa — Relatable Marathi customer care
    "tripti": "MwUMLXurEzSN7bIfIdXF",  # Tripti — Calm and experienced
}


@app.route("/api/tts", methods=["POST"])
def tts_endpoint():
    """Turns chatbot reply text into natural-sounding speech using ElevenLabs.
    Returns raw MP3 bytes on success. On any failure (no key, quota used up,
    no internet, bad request) it returns a 204 with no body — the frontend
    then quietly falls back to the browser's own built-in voice, so the
    chatbot's voice mode never breaks because of this. A short reason is
    also included in the X-TTS-Error response header for debugging (visible
    in the browser's Network tab under this request's Response Headers)."""
    api_key = os.environ.get("ELEVENLABS_API_KEY")
    if not api_key:
        return Response("", status=204, headers={"X-TTS-Error": "no-api-key-set"})

    body = request.get_json() or {}
    text = (body.get("text") or "").strip()
    if not text:
        return Response("", status=204, headers={"X-TTS-Error": "empty-text"})
    text = text[:800]  # keep requests small/fast and within free-tier character limits

    voice_key = (body.get("voice") or "").strip().lower()
    ...
    try:
        ...
        resp = requests.post(
            ...
            json={
                "text": text,
                "model_id": "eleven_multilingual_v2",  # handles Hindi/Marathi text too
        ...
        err_detail = resp.text[:200].replace("\n", " ").replace("\r", " ")
        print(f"ElevenLabs TTS error: {resp.status_code} - {resp.text[:300]}")
        return Response(
            "", status=204,
            headers={"X-TTS-Error": f"elevenlabs-{resp.status_code}: {err_detail}"}
        )
    except Exception as e:
        print(f"ElevenLabs TTS failed: {e}")
        return Response("", status=204, headers={"X-TTS-Error": f"exception: {str(e)[:200]}"})
