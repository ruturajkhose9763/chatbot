...
FALLBACK = (
    "Hmm, mujhe iska exact answer abhi nahi pata 😅 lekin koi baat nahi — thoda alag tarike se "
    "puchoge toh shayad main samajh paunga! Ya phir aap college office ko directly contact kar "
    "sakte ho, main unka number bhi bata sakta hoon. Waise main courses, admission, fees, "
    "facilities, placements, staff, timings, timetable, scholarship, hostel, exam schedule, "
    "news, events — in sab cheezon mein help kar sakta hoon. Kuch aur poochna hai? 😊"
)
...
def ai_hybrid_reply(message, data):
    ...
    try:
        ...
        prompt = (
            f"You are College AI Assistant, the warm and friendly enquiry chatbot for {college_name}. "
            "First try to answer the student's question using the JSON college data given below. "
            "If it's a general/casual message (small talk, a compliment, an off-topic question, "
            "something not covered by the data), respond warmly and naturally like a friendly "
            "helper would — never say you 'can't help' or leave the student without a reply. "
            "Only if it's a specific factual question about the college that truly isn't in the "
            "data should you politely say you don't have that exact detail and suggest they "
            "contact the college office. Always keep the conversation going in a friendly tone. "
...
def admin_dashboard():
    ...
    return render_template(
        "admin.html", college=college, current_account=current_account,
        analytics=analytics, custom_qa_items=custom_qa_items
    )
