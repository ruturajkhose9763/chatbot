...
def fmt_news(_d, branch=None, message=""):
    ...
    if news:
        ...
        for n in sorted(news, key=lambda x: x.get("date", ""), reverse=True)[:6]:
            lines.append(f"• [{n.get('date', '')}] {n['title']} — {n['description']}")
...
def fmt_events(_d, branch=None, message=""):
    ...
    for e in sorted(events, key=lambda x: x.get("date", "")):
        lines.append(f"• [{e.get('date', '')}] {e['title']} — {e['description']}")
    return "\n".join(lines)
...
def home():
    data = get_data()
    content = get_content()
    news = sorted(content.get("news", []), key=lambda x: x.get("date", ""), reverse=True)
    for n in news:
        try:
            n["is_new"] = (datetime.utcnow().date() - datetime.strptime(n["date"], "%Y-%m-%d").date()).days <= 3
        except (ValueError, KeyError):
            n["is_new"] = False
    events = sorted(content.get("events", []), key=lambda x: x.get("date", ""))
    notices = content.get("notices", [])
...
def _build_dashboard_context():
    content = get_content()
    admissions = sorted(get_admissions().get("submissions", []), key=lambda x: x.get("date", ""), reverse=True)
    accounts = get_accounts()["accounts"]
    college = get_data()
