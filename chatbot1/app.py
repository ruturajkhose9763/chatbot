...
def fmt_staff(d, branch=None, message=""):
    ...
    # If the question clearly asks about ONE specific role, answer just
    # that — don't dump the entire management list for a precise question.
    role_keywords = {
        "founder": ["founder"],
        "chairman_president": ["chairman", "president", "अध्यक्ष"],
        "vice_chairman": ["vice chairman", "vice-chairman", "vice president"],
    ...
    for role_key, kws in role_keywords.items():
        if any(kw in msg for kw in kws) and role_key in m:
            info = m[role_key]
            role_title = "CEO" if role_key == "ceo" else role_key.replace("_", " ").title()
            if isinstance(info, dict):
                name = info.get("name", "N/A")
                contact = info.get("contact_number", "N/A")
                return f"The {role_title} of the college is {name}.\nContact: {contact}"
            return f"The {role_title} of the college is {info}."
...
INTENTS = [
    ...
    {"name": "staff", "keywords": ["hod", "principal", "chairman", "staff", "teacher", "faculty", "management",
                     "director", "department", "dept", "विभाग", "ceo", "chief executive", "secretary",
                     "treasurer", "vice chairman", "vice-chairman", "vice president", "founder", "president",
...
def sitemap_xml():
    ...
    # Only genuinely public pages belong in the sitemap — admin/login pages
    # should never be indexed by Google.
    public_paths = [
        ("/", "1.0"),
        ("/chat", "0.8"),
        ("/#about", "0.6"),
...
def raw_data_view():
    d = get_data()
    pretty_json = json.dumps(d, indent=2, ensure_ascii=False)
    return render_template("raw_data.html", college=get_data(), raw_json=pretty_json, error=None)


@app.route("/admin/raw-data/save", methods=["POST"])
@main_admin_required
def raw_data_save():
    submitted_text = request.form.get("raw_json", "")
    try:
        parsed = json.loads(submitted_text)
    except json.JSONDecodeError as e:
        # Invalid JSON — show the error and the text they typed back to them,
        # WITHOUT touching the real saved data, so nothing on the live site breaks.
        return render_template(
            "raw_data.html", college=get_data(), raw_json=submitted_text,
            error=f"Invalid JSON — nothing was saved. Error: {e.msg} (near line {e.lineno}, column {e.colno})"
        )
    if not isinstance(parsed, dict):
        return render_template(
            "raw_data.html", college=get_data(), raw_json=submitted_text,
            error="The data must be a JSON object (starting with { and ending with }) — nothing was saved."
        )

    # Keep a one-step-back safety copy before overwriting, in case of mistakes.
    try:
        save_json(os.path.join(BASE_DIR, "college_info_backup.json"), get_data())
    except Exception:
        pass

    save_data(parsed)
    flash("Chatbot data saved successfully.")
    return redirect(url_for("raw_data_view"))
