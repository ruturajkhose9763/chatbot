...
def _build_dashboard_context():
    ...
    if my_class is not None:
        ...
        raw_roster = class_att.get("students", [])
        # Present one consistent roster shape to templates. Old name-only
        # entries are retained and displayed, while new accounts use roll_no.
        my_class_students = []
        ...
        profile_by_roll = {normalize_roll(s.get("roll_no")): s for s in profiles if s.get("roll_no")}
        for raw in raw_roster:
            if isinstance(raw, dict):
                roll = normalize_roll(raw.get("roll_no"))
                profile = profile_by_roll.get(roll) or raw
                my_class_students.append({
                    "roll_no": roll,
                    "name": profile.get("name") or raw.get("name", ""),
                    "mobile": profile.get("mobile", ""),
                    "mobile_verified": profile.get("mobile_verified", False),
                })
            else:
                name = str(raw).strip()
                profile = profile_by_name.get(name.lower(), {})
                my_class_students.append({
                    "roll_no": normalize_roll(profile.get("roll_no")),
                    "name": name,
                    "mobile": profile.get("mobile", ""),
                    "mobile_verified": profile.get("mobile_verified", False),
                })
        my_class_student_records = list(my_class_students)
        records = class_att.get("records", {}) if isinstance(class_att.get("records", {}), dict) else {}
        my_class_attendance_dates = sorted(records.keys(), reverse=True)[:10]

        # Attendance percentages are calculated by roll number for new records,
        # with a legacy name fallback for old records.
        for student in my_class_students:
            roll_no = normalize_roll(student.get("roll_no"))
            ...
            for day_status in records.values():
                if not isinstance(day_status, dict):
                    continue
                status = day_status.get(roll_no) or day_status.get(student_name)
                if status in ("present", "absent"):
                    total_days += 1
                    if status == "present":
                        present_days += 1
            pct = round((present_days / total_days) * 100) if total_days else 0
            my_class_attendance_pct.append({
                "name": student_name, "roll_no": roll_no,
                "pct": pct, "present_days": present_days, "total_days": total_days
            })

        for student in my_class_students:
            roll_no = normalize_roll(student.get("roll_no"))
            student_name = student.get("name", "")
            row = {"name": student_name, "roll_no": roll_no, "statuses": []}
            for d in my_class_attendance_dates:
                day = records.get(d, {}) or {}
                row["statuses"].append(day.get(roll_no) or day.get(student_name) or "-")
...
def attendance_add_student():
    """Create/update a real Student Portal account and add it to the teacher's class roster."""
    account = find_account(session["username"])
    ...
    if existing:
        ...
        target = existing
        message = f"Student {student_name} ({roll_no}) updated."
    else:
        target = {
        ...
        students_data["students"].append(target)
        message = f"Student account created for {student_name} ({roll_no})."

    # Keep a single clean roster entry per roll number. New records use the
    # student ID/roll number as the attendance key; old name-based records are
    # still read by the dashboard for backward compatibility.
    attendance = get_attendance() or {"classes": {}}
    attendance.setdefault("classes", {})
    cls = attendance["classes"].setdefault(class_key, {"students": [], "records": {}})
    roster = cls.setdefault("students", [])
    normalized_roster = []
    found = False
    for item in roster:
        if isinstance(item, dict):
            item_roll = normalize_roll(item.get("roll_no"))
            if item_roll == roll_no:
                if not found:
                    normalized_roster.append({"roll_no": roll_no, "name": student_name})
                    found = True
            else:
                normalized_roster.append(item)
        else:
            old_name = str(item).strip()
            # Preserve legacy students as strings so old attendance remains readable.
            if old_name.lower() == student_name.lower() and not found:
                normalized_roster.append({"roll_no": roll_no, "name": student_name})
                found = True
            else:
                normalized_roster.append(old_name)
    if not found:
        normalized_roster.append({"roll_no": roll_no, "name": student_name})
    cls["students"] = normalized_roster
    attendance.setdefault("schema_version", 2)

    save_students(students_data)
    save_attendance(attendance)

    # Every account with a mobile number starts pending verification. Send a
    # fresh OTP on creation or when the mobile number was changed.
    ...
    if target and target.get("mobile") and not target.get("mobile_verified", False):
        ...
        if _send_teacher_otp_sms(target["mobile"], otp):
            flash(message + " Verification OTP sent to the registered mobile number.")
        else:
            flash(message + " OTP is pending; configure Twilio to send the SMS.")
            print(f"[STUDENT OTP - {roll_no}] {otp}")
    else:
        flash(message)
    return redirect_to_dashboard()
...
def attendance_remove_student():
    ...
    attendance = get_attendance() or {"classes": {}}
    cls = attendance.setdefault("classes", {}).setdefault(class_key, {"students": [], "records": {}})
    new_roster = []
    for item in cls.get("students", []):
        if isinstance(item, dict):
            if roll_no and normalize_roll(item.get("roll_no")) == roll_no:
                continue
            if not roll_no and student_name and item.get("name") == student_name:
                continue
        elif student_name and str(item).strip() == student_name:
            continue
        new_roster.append(item)
    cls["students"] = new_roster
    save_attendance(attendance)
    flash(f"Student {roll_no or student_name} removed from the class roster.")
    return redirect_to_dashboard()
...
def attendance_mark():
    ...
    for raw_student in cls.get("students", []):
        if isinstance(raw_student, dict):
            roll_no = normalize_roll(raw_student.get("roll_no"))
            student_name = (raw_student.get("name") or "").strip()
        else:
            roll_no = ""
            student_name = str(raw_student).strip()

        key = roll_no or student_name
        status = request.form.get(f"status_{key}", "absent")
        if status not in ("present", "absent"):
            status = "absent"
        day_record[key] = status

        profile = find_student(roll_no) if roll_no else find_student_by_name_class(student_name, class_key)
        if profile:
    ...
    cls.setdefault("records", {})[att_date] = day_record
    save_attendance(attendance)
    save_students(students_data)
...
def student_dashboard():
    ...
    for date_key in sorted(records.keys(), reverse=True):
        ...
        status = day.get(roll_no) or day.get(name)  # roll-number records first; legacy name records supported
        if status in ("present", "absent"):
            total += 1
