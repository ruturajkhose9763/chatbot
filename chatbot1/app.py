...
def get_subscribers():
    if not data_exists(SUBSCRIBERS_PATH):
        save_json(SUBSCRIBERS_PATH, {"emails": []})
    return load_json(SUBSCRIBERS_PATH)


def save_subscribers(data):
    save_json(SUBSCRIBERS_PATH, data)


def send_email(to_email, subject, body):
    """Send one email via SMTP. Silently does nothing if SMTP_EMAIL /
    SMTP_APP_PASSWORD aren't set as environment variables on Render."""
...
def notify_subscribers(subject, body):
    """Email every subscriber in a background thread so posting a notice
    never feels slow, even if SMTP is slow or times out."""
    emails = get_subscribers().get("emails", [])
    if not emails:
        return
...
def send_sms(phone, message):
    """Placeholder for SMS — wire this up to Fast2SMS / Twilio once you
    have a paid API key; until then this safely does nothing."""
    api_key = os.environ.get("SMS_API_KEY")
    if not api_key:
        return False
    # Example (Fast2SMS): uncomment and adjust once you have a key.
    # import requests
...
def subscribe():
    ...
    if not email or "@" not in email:
        ...
        return redirect(url_for("home") + "#notice-board")

    data = get_subscribers()
    if email not in data["emails"]:
        data["emails"].append(email)
        save_subscribers(data)
        send_email(email, "Subscribed to Notices",
                   "You're now subscribed to notice updates from the college website. "
                   "You'll get an email whenever a new notice is posted.")
    flash("Subscribed! You'll get an email whenever a new notice is posted.")
    return redirect(url_for("home") + "#notice-board")
...
def unsubscribe(email):
    ...
    if email in data["emails"]:
        ...
        save_subscribers(data)
    flash("You've been unsubscribed from notice emails.")
    return redirect(url_for("home"))
...
def add_notice():
    ...
    new_id = (max([n["id"] for n in notices], default=0)) + 1
    notice_text = request.form.get("text", "").strip()
    ...
    college_name = get_data().get("college_name", "College")
    subject_line = f"{'🔴 URGENT — ' if priority == 'urgent' else ''}New Notice from {college_name}"
    notify_subscribers(subject_line, notice_text)

    flash("Notice posted." + (" Subscribers are being emailed." if get_subscribers().get("emails") else ""))
    return redirect_to_dashboard()
