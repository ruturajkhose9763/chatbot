...
def fmt_seats(d, branch=None, message=""):
    ...
    if branch:
        ...
        c = courses[0]
        return (f"In {c['name']}, there are currently {c['seats']} seats for 1st year, "
                f"and {c.get('direct_second_year_seats', 'N/A')} seats available for Direct 2nd Year entry. 😊")
    lines = ["Here's the seat intake for each branch:"]
    for c in courses:
...
def fmt_admission(d, branch=None, message=""):
    ...
    if mentions_any(msg, ["eligibility", "patrata", "पात्रता", "eligible"]):
        return f"Sure! For eligibility — {a.get('eligibility', 'N/A')}"

    if mentions_any(msg, ["minimum marks", "kitne percent", "kitna percentage", "% marks"]):
        return f"The minimum marks required are: {a.get('minimum_marks', 'N/A')}"

    if mentions_any(msg, ["how to apply", "kaise apply", "apply kaise", "kaise le", "kaise admission le"]):
        return f"Here's how you can apply — {a.get('how_to_apply', 'N/A')}"

    if mentions_any(msg, ["entrance", "cap", "merit"]):
        return f"For the entrance process — {a.get('entrance_exam', 'N/A')}"

    if mentions_any(msg, ["institute code", "college code"]):
        return f"Our Institute Code is {a.get('institute_code', 'N/A')}."
...
def fmt_placements(d, branch=None, message=""):
    ...
    if mentions_any(msg, ["package", "salary", "lpa"]) and not mentions_any(msg, ["company", "companies"]):
        return (f"Our students get an average package of {p.get('average_package', 'N/A')}, "
                f"with the highest package going up to {p.get('highest_package', 'N/A')}, "
                f"and a starting package of around {p.get('starting_package', 'N/A')}. 💼")
...
def fmt_contact(d, branch=None, message=""):
    c = d.get("contact", {})
    return (
        f"You can reach us at {c.get('address', 'N/A')}. "
        f"Call us on {c.get('phone', 'N/A')} or email {c.get('email', 'N/A')}. "
...
def fmt_timings(d, branch=None, message=""):
    ...
    return (
        ...
        f"On Saturdays, it's usually {t.get('saturday', 'N/A').lower() if t.get('saturday') else 'as per schedule'}."
    )
...
def fmt_staff(d, branch=None, message=""):
    ...
    if branch:
        ...
        if matched_key:
            ...
            branch_title = matched_key.replace('_', ' ').title()
            contact = info.get('contact_number')
            email = info.get('email')
            reply = f"The HOD of {branch_title} is {info.get('hod_name', 'N/A')}."
            if contact and contact != 'N/A' and contact.lower() != 'not available':
                reply += f" You can reach them at {contact}."
            if email and email != 'N/A' and email.lower() != 'not available':
                reply += f" Email: {email}."
...
def ai_hybrid_reply(message, data, history=None):
    ...
    try:
        ...
        system_prompt = (
            ...
            "For questions about the college, answer using the JSON college data given below. "
            "If it's a general question unrelated to the college (general knowledge, casual "
            "chit-chat, how-to questions, etc.), feel free to answer it normally and helpfully "
            "using your own knowledge, just like a helpful assistant would — don't refuse those. "
            "Reply in the same mixed Hinglish/English style as the question, keep it short "
            f"(under 80 words unless the question genuinely needs more). Use the earlier "
            f"conversation turns (if any) for context, e.g. follow-up questions like "
            f"'what about civil?' or 'which is cheaper?'.\n\n"
            f"College data:\n{context_summary}"
        )
...
def openrouter_reply(message, data, history=None):
    ...
    try:
        ...
        system_prompt = (
            ...
            f"For questions about the college, answer using this college data, and nothing else "
            f"— never guess or invent college-specific details: {context_summary}. "
            f"If it's a general question unrelated to the college (general knowledge, casual "
            f"chit-chat, how-to questions, etc.), feel free to answer it normally and helpfully "
            f"using your own knowledge, just like a helpful assistant would — don't refuse those. "
            f"Keep the reply short (under 80 words unless the question genuinely needs more), "
            f"friendly, in Hinglish/English."
        )
