...
def translate_for_speech(text, target_lang):
    """Translate `text` into the language the user picked for the voice
    assistant (Hindi / English / Marathi), so tapping "Hindi voice" gives a
    real Hindi answer even if the underlying reply text was in English —
    not just an accent change. Uses the same free-tier Gemini key as the
    AI hybrid fallback. If no key is set, or the call fails for any reason
    (offline, rate limit, etc.), the original text is returned unchanged
    so speech never breaks."""
    api_key = os.environ.get("GEMINI_API_KEY")
    if not api_key:
        return text
    lang_names = {"hi": "Hindi (Devanagari script)", "en": "English", "mr": "Marathi (Devanagari script)"}
    ...
    try:
        import requests
        prompt = (
            f"Translate the following college-chatbot reply into natural, spoken {target_name}. "
            "Keep numbers, phone numbers, fees amounts, and course names as-is. "
...
@app.route("/api/translate-for-speech", methods=["POST"])
def translate_for_speech_route():
    body = request.get_json() or {}
    text = (body.get("text") or "").strip()
    target_lang = (body.get("lang") or "").strip()
    if not text or target_lang not in ("hi", "en", "mr"):
        return jsonify({"text": text})
    translated = translate_for_speech(text, target_lang)
